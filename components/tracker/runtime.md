---
name: Node service runtime
summary: Exact npm dependency and official MCP runtime contract
kind: runtime
---
# Node service runtime

### Requirement: Exact npm authority

The application SHALL use the exact following package manifest and complete npm lock.
...
explicit persistent-service operations.
...
#### Scenario: Dependency closure replay

- **WHEN** the authorized lifecycle runs npm ci against these exact files
- **THEN** it installs the same package versions and verifies the same dependency graph

### Requirement: Official MCP implementation

Use Server from @modelcontextprotocol/sdk/server/index.js, protocol schemas from
@modelcontextprotocol/sdk/types.js, StreamableHTTPServerTransport from
@modelcontextprotocol/sdk/server/streamableHttp.js and StdioServerTransport from
@modelcontextprotocol/sdk/server/stdio.js. Register typed list/call tools and list/read
resources handlers with the official SDK. Connect and close per-request stateless HTTP
transports correctly, and enforce the same HTTP authentication before MCP transport
