...
with tempfile.TemporaryDirectory(prefix='tracker-expiry-') as data:
    ...
    try:
        ...
        with sync_playwright() as playwright:
            ...
            active_badge = repo_control.locator('.badge').filter(has_text=re.compile(r'\bactive\b', re.I))
            sidebar_text = active_badge.inner_text() if active_badge.count() else repo_control.inner_text()
            assert re.search(r'\b1\b', sidebar_text), sidebar_text
            ...
            result = {'elapsed_seconds': elapsed, 'row': row.inner_text(),
                      'sidebar': active_badge.inner_text() if active_badge.count() else repo_control.inner_text(), 'stale_visible': stale.is_visible()}
